...
class Cifar(object):
	...
	def setup(self, training):
		...
		if training:
			...
			# final loss function
			self.losses = tf.add_n([l2_loss, cross_entropy], name='loss')
			### END CODE HERE
	...
	def train(self, x_train, y_train, max_epoch):
		...
		for epoch in range(1, max_epoch+1):
			...
			learning_rate = 0.1 if epoch < int(0.8 * max_epoch) else 0.01
			### END CODE HERE
			...
			for i in range(num_batches):
				...
				xx_batch = curr_x_train \
					[self.conf.batch_size * i:self.conf.batch_size * (i + 1)]
				y_batch = curr_y_train \
					[self.conf.batch_size * i:self.conf.batch_size * (i + 1)]

				x_batch = np.zeros(shape=(self.conf.batch_size, 32, 32, 3))

				for i in range(self.conf.batch_size):
					x_batch[i] = parse_record(xx_batch[i], training=1)
				### END CODE HERE
	...
	def test_or_validate(self, x, y, checkpoint_num_list):
		...
		for checkpoint_num in checkpoint_num_list:
			...
			for i in tqdm(range(x.shape[0])):
				### YOUR CODE HERE
				inputs = parse_record(x[i], training=0).reshape((1, 32, 32, 3))
