...
class ResNet(object):
	...
	def _batch_norm_relu(self, inputs, training):
		...
		### YOUR CODE HERE
		inputs_norm = tf.layers.batch_normalization(inputs, training=training)
		outputs = tf.nn.relu(inputs_norm)
		### END CODE HERE
	...
	def _start_layer(self, inputs, training):
		...
		### YOUR CODE HERE
		# initial conv1
		outputs = tf.layers.conv2d(inputs=inputs, filters=self.first_num_filters, kernel_size=3, padding='same', strides=1, kernel_initializer=tf.variance_scaling_initializer())
		### END CODE HERE
	...
	def _output_layer(self, inputs, training):
		...
		outputs = tf.layers.dense(avg_pool, self.num_classes)
		#outputs = tf.layers.dense(avg_pool, self.num_classes, activation=tf.nn.softmax)
		### END CODE HERE
	...
	def _stack_layer(self, inputs, filters, block_fn, strides, training):
		...
		def projection_shortcut(inputs):
			### YOUR CODE HERE
			return tf.layers.conv2d(inputs=inputs, filters=filters_out, kernel_size=1, strides=strides, kernel_initializer=tf.variance_scaling_initializer())
			### END CODE HERE
	...
	def _standard_block_v1(self, inputs, filters, training, projection_shortcut, strides):
		...
		outputs = tf.layers.conv2d(
			kernel_size=3, strides=strides, filters=filters,
			kernel_initializer=tf.variance_scaling_initializer(),
			use_bias=False, padding='same', inputs=inputs
		)

		outputs = self._batch_norm_relu(outputs, training)

		outputs = tf.layers.conv2d(
			kernel_size=3, strides=1, filters=filters,
			kernel_initializer=tf.variance_scaling_initializer(),
			use_bias=False, padding='same', inputs=outputs
		)
	...
	def _bottleneck_block_v2(self, inputs, filters, training, projection_shortcut, strides):
		...
		### YOUR CODE HERE
		outputs = tf.layers.conv2d(
			kernel_size=1, strides=1, filters=filters,
			kernel_initializer=tf.variance_scaling_initializer(),
			use_bias=False, inputs=outputs
		)
		...
		outputs = tf.layers.conv2d(
			kernel_size=3, strides=strides, filters=filters,
			kernel_initializer=tf.variance_scaling_initializer(),
			use_bias=False, inputs=outputs, padding='same'
		)

		outputs = self._batch_norm_relu(outputs, training)

		outputs = tf.layers.conv2d(
			kernel_size=1, strides=1, filters=4 * filters,
			kernel_initializer=tf.variance_scaling_initializer(),
			use_bias=False, inputs=outputs)
